...
public class phase2_random_generator {
    ...
    public phase2_random_generator(List<MetamorphicGroup> mgDomain) throws IOException {
        this("src/main/java/paper/pss/exp/math2_project/math2_config.json", mgDomain);
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            List<MetamorphicGroup> cases = sampler.generate(5);

            System.out.println("随机抽取的测试用例:");
            for (int i = 0; i < cases.size(); i++) {
                MetamorphicGroup mg = cases.get(i);
                System.out.println(String.format("RT测试用例 #%d: MR=%s, STC=(%d,%d), FTC=(%d,%d)",
                        i + 1,
                        mg.getMRId(),
