package paper.pss.exp.jfreeChart_project.mutants_analysis;
...
public class MutantAnalysis {
    ...
    public void loadMutants() {
        mutantNames.clear();
        // 扫描mutant目录
        Path mutantsPath = Paths.get("src/main/java/paper/pss/exp/jfreeChart_project/mutants");
        ...
        try {
            List<String> mutants = Files.list(mutantsPath)
                    ...
                    .sorted((a, b) -> {
                        try {
                            // 提取数字部分进行排序
                            String numStrA = a.replaceAll("[^0-9]", "");
                            String numStrB = b.replaceAll("[^0-9]", "");
                            if (!numStrA.isEmpty() && !numStrB.isEmpty()) {
    ...
    public void executeAllTests() {
        ...
        for (String mutantName : mutantNames) {
            ...
            try {
                ...
                Method mutantMethod = mutantClass.getMethod("createLineRegion", Line2D.class, float.class);
                ...
                for (TestCase testCase : testCases) {
                    ...
                    if ((sourceResult instanceof TimeoutException) || (followupResult instanceof TimeoutException)) {
                        ...
                        // 验证 group是否满足蜕变关系
                        if (sourceResult instanceof Shape && followupResult instanceof Shape) {
    ...
    private String getTestCaseKey(TestCase testCase) {
        ...
        return String.format("Line[(%f,%f)-(%f,%f)]_width_%f",
                line.getX1(), line.getY1(), line.getX2(), line.getY2(), testCase.getWidth());
    }
    ...
    private Object executeWithTimeout(Method method, TestCase testCase) {
        ...
        try {
            Future<Object> future = executor.submit(() -> method.invoke(null, line, width));
    ...
    private Object executeWithoutTimeout(Method method, TestCase testCase) {
        ...
        try {
            // 直接调用反射方法
            return method.invoke(null, line, width);
        } catch (Exception e) {
            // 捕获并打印异常，方便调试
        ...
        try {
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
                ...
                for (TestCase testCase : testCases) {
                    ...
                    writer.print("\"" + testCaseKey + "\"");

                    for (String mutantName : mutantNames) {
                        MutantResult result = mutantResults.get(mutantName);
                        Object res = result.results.get(testCaseKey);
                        String status = result.statuses.get(testCaseKey);
    ...
    public static void main(String[] args) {
        try {
            System.out.println("🚀 开始简化版突变体分析 - JFreeChart项目...");

            MutantAnalysis analyzer = new MutantAnalysis();
            int testCaseCount = 6 * 1000; // 默认值
            ...
            analyzer.generateReport(
                    "src/main/java/paper/pss/exp/jfreeChart_project/mutants_analysis/mutant_analysis_report.json");

            // 保存结果
            analyzer.saveResults(
                    "src/main/java/paper/pss/exp/jfreeChart_project/mutants_analysis/simplified_mutant_analysis.csv");
