...
from python.incomeTax.generation.phase1.random_generator import RandomGenerator
...
class MutantAnalysis:
    ...
    def _load_config(self) -> Dict:
        ...
        config_path = os.path.join(
            os.path.dirname(__file__), "../../incomeTax_config.json"
        )
        try:
    ...
    def generate_test_cases(self, count: int = 1000) -> List[TestCase]:
        ...
        try:
            ...
            raise RuntimeError(f"生成测试用例失败: {str(e)}") from e

        return self.test_cases

    def load_mutants(self) -> None:
        """
        发现并加载突变体（使用test_subject字典）
        """
        self.mutant_names.clear()

        # 从test_subject字典获取突变体
        mutant_methods = []
        for name in self.mutants_instance.test_subject.keys():
    ...
    def _execute_with_timeout(self, func, test_case: TestCase) -> Tuple[Any, str]:
        ...
        Returns:
            Tuple[Any, str]: (结果, 状态)
        """
        income_value = test_case.get_income_value()

        try:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(func, income_value)
    ...
    def execute_all_tests(self) -> None:
        ...
        # 获取原始函数
        original_func = self.mutants_instance.origin
        ...
        for mutant_name in self.mutant_names:
            ...
            try:
                ...
                for test_case in self.test_cases:
                    input_str = str(test_case.get_income_value())
    ...
    def calculate_maximum_independent_set(self) -> List[str]:
        ...
        for mutant_name in self.mutant_names:
            result = self.mutant_results[mutant_name]
            if result.type == MutantType.NORMAL:
                normal_mutants.append(mutant_name)

        if not normal_mutants:
            return []

        # 使用贪心算法计算最大独立集
        return self._find_maximum_independent_set_greedy(normal_mutants)
    ...
    def generate_report(self, filename: Optional[str] = None) -> Dict:
        ...
        # 计算突变体分数
        effective = len(type_groups[MutantType.NORMAL])
        score = (effective / len(self.mutant_names) * 100) if self.mutant_names else 0
        report["statistics"]["mutation_score"] = round(score, 2)

        # 添加包含关系信息
        subsumption_relations = []
        for mutant_name in self.mutant_names:
            result = self.mutant_results[mutant_name]
            for subsumed in result.subsumes:
                subsumption_relations.append(
                    {"subsumer": mutant_name, "subsumed": subsumed}
                )
