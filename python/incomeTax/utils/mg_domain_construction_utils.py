...
from python.incomeTax.model.metamorphic_relation import MetamorphicRelationFactory
...
from python.incomeTax.generation.phase1.partition_generator import PartitionGenerator
...
class MGDomainGenerator:
    ...
    def __init__(self, domain_size: int = 1000):
        """
        初始化蜕变组域生成器
        ...
        self.config_extractor = IncomeTaxConfigExtractor(self.config_path)
    ...
    def generate_domain(self) -> List[MetamorphicGroup]:
        ...
        """
        # 1. 生成源测试用例
        source_tests = self.partition_generator.generate(self.domain_size)
    ...
    def calculate_domain_size(self, min_samples_per_category: int = 5) -> int:
        ...
        return min(max(suggested_size, 1000), 20000)  # 最小5000，最大20000
    ...
    def get_domain_statistics(self, mg_domain: List[MetamorphicGroup]) -> str:
        ...
        partition_list = sorted(list(partition_set))
